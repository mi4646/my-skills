...
- **本地路径**：`/home/anonymous/agent-skills/`（2026-09-14 实测；原记的 `~/skills/addyosmani/` 已不存在）
- **HEAD**：`be4e44a`（2026-09-11 提交；2026-09-14 实测，原记 `d2478bf` 已过时）
- **git remote**：`https://github.com/addyosmani/agent-skills.git`
- **版本**：0.6.6（MIT）
- **规模**：25 skill + 4 agent + 8 slash command + hooks + evals 框架（2026-09-14 实测）
- **形态**：Claude Code marketplace 插件（`plugin.json` + `.claude-plugin/marketplace.json`）

## 目录结构
- `skills/<name>/SKILL.md`：25 个核心 skill，按 SDLC 六阶段组织（Define/Plan/Build/Verify/Review/Ship + Meta）
...
3. **手动复制**：`cp -r skills/<name> ~/.claude/skills/<name>`（2026-08-07 起采用此方式，现共四项已装）
...
- install.sh ③段按独立 skill 目录形态管理（从远程 clone 到临时缓存 `$VENDOR_DIR/addyosmani`，非插件目录；原记「上游固定 `~/skills/addyosmani/`」已作废）；`--list` 支持第三字段统计
